...
test('referral_codes: leaderは作成可・memberは閲覧のみ可・別組織からは見えない・org内で一意', async ({ request }) => {
  ...
  // 6. 同組織の別leaderは他leaderが作成したコードを削除できない(0022レビュー指摘の回帰確認)
  const leader2DeleteRes = await request.delete(`${LOCAL_SUPABASE_URL}/rest/v1/referral_codes?id=eq.${rc!.id}`, {
  ...
  expect(leader2DeleteBody.length).toBe(0);
  const stillExistsRes = await request.get(`${LOCAL_SUPABASE_URL}/rest/v1/referral_codes?id=eq.${rc!.id}&select=id`, { headers: leader.authHeaders });
