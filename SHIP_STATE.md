...
### 3. F-01〜F-06 AC細目の照合（完了 ✅）
`requirements_mvp.md` §5の全25 AC項目をコードと突合。23項目は既存実装で確認済み
（F-01顧客CRUD+絞り込み・F-02差分質問・F-03文字起こしパイプライン・F-04相談チャット・
F-05リーダー閲覧・F-06認証/テナント分離/課金/IAP回避、全て確認）。

2件のギャップを発見・対応:
- **温度感タップ入力がおさらい対話画面に無い** → 対話完了後のサマリ確認画面に統合して解決（下記）。
- **AIサマリの確認・修正ステップが無い**（自動保存のみ）→ 対応。

**実装**（コミット `311f85e`）: おさらい対話完了(done)後、自動保存はそのまま維持しつつ
編集可能なサマリ確認画面を追加。
- `apps/mobile/src/screens/Osarai.tsx`: 要点/ニーズ/次アクション(複数行テキスト)＋
  温度感タップ選択(CustomerForm.tsxと同じUI)を表示、「この内容で保存」で確定。
  これにより2つのAC gapを1つの画面で同時に解消。
- `apps/mobile/src/lib/db.ts`: `updateInteractionSummary()`追加。RLSスコープ済み
  クライアントで`interactions.ai_summary`/`customers.temperature,needs`を直接PATCH。
- Playwright(`osarai-summary-edit.spec.ts`): mobileにE2Eハーネスが無いため、リスクの
  核心である「本人のinteraction/customerへのPATCHがRLSで許可されるか」をweb側から
  直接検証（同じPATCH操作を再現）。
- Gemini独立レビュー: findings 1件(medium・保存ボタンの冪等性)。UPDATE操作で最終値が
  同じため多重実行しても実害小、かつUI側で`confirming`による連打防止済み。verdict=pass、対応不要と判断。
- typecheck/build green（作業中に発覚したnode_modules破損＝`vite/client.d.ts`欠損は
  完全クリーンインストールで解消。原因はpnpm storeの内容アドレス管理と、複数回の
  `rm -rf`操作の相性と推測）。
