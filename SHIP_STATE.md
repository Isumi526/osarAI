...
**🔴 デプロイ前に必ず解消すべきブロッカー**
- **migration 0005が本番Supabaseに未適用**。現状の本番プロジェクトは`profiles`/`subscriptions`等0001-0004相当のテーブルまで存在するが、`stripe_webhook_events`テーブルは存在しない（`subscriptions.last_stripe_event_at`列も無い）。**今回のA2修正後のwebhookコードは`stripe_webhook_events`へのinsertを前提にしている**ため、0005未適用のままデプロイするとStripe webhookが届くたびに500エラーになる（冒頭のdedupe insertが失敗）。適用はT6ルール上、追加のみDDL＋人の明示承認＋`SUPABASE_PROD_DB_URL`経由のpsqlでのみ可能（`db push`は常時禁止）。**CCはこの適用を実行していない**（`SUPABASE_PROD_DB_URL`未設定・実行には人の明示指示が必要）。
...
### 残っている「ボタン」（人が押す）
1. 0005 migrationを本番Supabaseへ適用（psql経由・要`SUPABASE_PROD_DB_URL`）
2. `vercel link` → Vercelプロジェクト作成（Root Directory=apps/web）
3. Vercel環境変数の設定（機密値は人が直接入力）
4. `vercel deploy --prod`
5. Stripe webhookエンドポイントを実URLで登録 → `STRIPE_WEBHOOK_SECRET`取得 → Vercel環境変数に追加 → 再デプロイ
6. （実決済を開始する時のみ）Stripe live鍵への切替
